...
final class ProductListHostController: UIHostingController<ProductListView> {
    ...
    init() {
        let viewModel = ProductListViewModel()
        self.viewModel = viewModel
        super.init(rootView: ProductListView(viewModel: viewModel))
    }
...
private extension ProductListHostController {
    func handleRouter(_ router: ProductListViewModel.Router) {
        switch router {
        case let .toDetail(product):
            AppRouter.shared.to(
                ProductDetailHostController(product: product),
                from: self
            )
        }
    }
...
導航邏輯全在這裡，`ProductListView` 完全不知道 `ProductDetailHostController` 的存在。所有導航統一走 `AppRouter.shared`，HostController 不直接碰 `navigationController`。

---

## AppRouter：讓導航有一個家

四層架構解決了「誰做什麼」的問題，但還有一個問題：HostController 要怎麼導航？

最直覺的做法是直接呼叫 `navigationController?.pushViewController`，但這樣每個 HostController 都在各自處理導航細節——轉場動畫怎麼寫、手勢怎麼接、sheet 和 push 的 dismiss 邏輯各自為政。功能一多，導航邏輯就開始四散。

所以我在 C 層之外多設計了一個 `AppRouter`，讓所有導航統一走這裡：
...
AppRouter 是無狀態的，不持有任何 stored property，nav controller 從 `source.navigationController` 動態取得。轉場動畫透過 `UINavigationControllerDelegate` 統一管理，手勢也在這裡統一處理。

HostController 不需要知道怎麼 push、怎麼 present、怎麼 dismiss——只要告訴 AppRouter 要去哪裡，剩下的交給它。這讓「導航決策」和「導航執行」分開，改一個轉場效果只需要動 AppRouter，不用翻每一個 HostController。
...
- [shinrenpan/MVVMC](https://github.com/shinrenpan/MVVMC) — 完整架構規範、Demo 專案、MCP Server
